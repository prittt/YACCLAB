...
__device__ __inline__ static void detectRuns512(ushort* pt, uchar* ct, uchar* ps, uchar* px, ushort* tbf, uint cp2)
{
	uint* ps4 = (uint*)ps;
	uint* pt2 = (uint*)pt;
...
//	uint cp2 = cp << 1;
	...
	t0 |= t0 >> 15;
	t0 |= t0 >> 6;
	t0 &= 0xf;	// now t0 has the 4 pixels in its 4 less significant bits
	t1 = px[cp];
	t5 = 0;
	if ((t1 & 0x3))// && (t0 & 0x3)
		t5 = 0x1;
...
__global__ void static pass1_512(ushort* pt, uchar* ps, ushort* b, ushort* b2, uint h, ushort* eb)
{
...
__global__ void static pass2_512(ushort* ib, uint* glabel, uint h, uint pitch)
{
...
__global__ void static pass3_512(ushort* pt, ushort* ps, ushort* b, uint* label, uint h)
{
