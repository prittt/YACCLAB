...
public:
    ...
    void PerformLabelingBlocksize(int x, int y, int z) override {
        ...
        grid_size_ = dim3(((d_img_.x + 1) / 2 + x - 1) / x, ((d_img_.y + 1) / 2 + y - 1) / y, ((d_img_.z + 1) / 2 + z - 1) / z);
        block_size_ = dim3(x, y, z);

        BLOCKSIZE_KERNEL(InitLabeling,grid_size_, block_size_, 0, d_img_labels_)

        BLOCKSIZE_KERNEL(Merge, grid_size_, block_size_, 0, d_img_, d_img_labels_, last_cube_fg_)

        BLOCKSIZE_KERNEL(PathCompression, grid_size_, block_size_, 0, d_img_labels_)

        BLOCKSIZE_KERNEL(FinalLabeling, grid_size_, block_size_, 0, d_img_, d_img_labels_, last_cube_fg_)
...
REGISTER_KERNELS(BUF_3D, InitLabeling, Merge, PathCompression, FinalLabeling)