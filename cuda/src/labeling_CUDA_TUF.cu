		...
		if (in_limits) {
			...
			if (v) {
				...
				if (local_row > 0 && s_img[local_index - BLOCK_COLS]) {
					...
					if (local_row > 0 && local_col < BLOCK_COLS - 1 && s_img[local_index - BLOCK_COLS + 1] == 1) {
						Union(s_buf, local_index, local_index - BLOCK_COLS + 1);

						if (local_row > 0 && local_col > 0 && s_img[local_index - BLOCK_COLS - 1]) {
							Union(s_buf, local_index, local_index - BLOCK_COLS - 1);
						}
						else {
		...
		if (in_limits) {
			...
			if (v) {
				...
				if (global_row > 0 && local_row == 0 && img[img_index - img.step] == 1) {
					...
					if (global_row > 0 && (global_col < img.cols - 1) && (local_row == 0 || local_col == BLOCK_COLS - 1) && img[img_index - img.step + 1] == 1) {
						Union(labels.data, labels_index, labels_index - labels.step / sizeof(int) + 1);

						if (global_row > 0 && global_col > 0 && (local_row == 0 || local_col == 0) && img[img_index - img.step - 1] == 1) {
							Union(labels.data, labels_index, labels_index - labels.step / sizeof(int) - 1);
						}
						else {
...
using namespace CUDA_TUF_namespace;

class CUDA_TUF : public GpuLabeling {
private:
	dim3 grid_size_;
	dim3 block_size_;
