...
//Si potrebbe provare con un albero o con tutti e 8
...
__global__ void LocalMerge(const cuda::PtrStepSzb img, cuda::PtrStepSzi labels) {
	...
	if (in_limits) {
		...
		if (local_col > 0 && s_img[local_index - 1] == v) {
			Union(s_buf, local_index, local_index - 1);
		}


		if (local_row > 0 && s_img[local_index - BLOCK_COLS] == v) {
			Union(s_buf, local_index, local_index - BLOCK_COLS);
		}


		if (local_row > 0 && local_col > 0 && s_img[local_index - BLOCK_COLS - 1] == v) {
			Union(s_buf, local_index, local_index - BLOCK_COLS - 1);
		}


		if (local_row > 0 && local_col < BLOCK_COLS - 1 && s_img[local_index - BLOCK_COLS + 1] == v) {
			Union(s_buf, local_index, local_index - BLOCK_COLS + 1);
		}
	...
	if (in_limits) {
		...
		labels.data[global_row * labels.step / sizeof(int) + global_col] = global_f;		// Nessuna distinzione tra background e foreground
	}
}
...
__global__ void GlobalMerge(const cuda::PtrStepSzb img, cuda::PtrStepSzi labels) {
	...
	if (in_limits) {
		...
		if (global_col > 0 && local_col == 0 && img[img_index - 1] == v) {
			Union(labels.data, labels_index, labels_index - 1);
		}

		if (global_row > 0 && local_row == 0 && img[img_index - img.step] == v) {
			Union(labels.data, labels_index, labels_index - labels.step / sizeof(int));
		}

		if (global_row > 0 && global_col > 0 && (local_row == 0 || local_col == 0) && img[img_index - img.step - 1] == v) {
			Union(labels.data, labels_index, labels_index - labels.step / sizeof(int) - 1);
		}

		if (global_row > 0 && (global_col < img.cols - 1) && (local_row == 0 || local_col == BLOCK_COLS - 1) && img[img_index - img.step + 1] == v) {
			Union(labels.data, labels_index, labels_index - labels.step / sizeof(int) + 1);
		}
	}
}
...
__global__ void PathCompression(const cuda::PtrStepSzb img, cuda::PtrStepSzi labels, cuda::PtrStepSzi labels_definitive) {
	...
	if (global_row < labels.rows && global_col < labels.cols) {
		...
		// L'operatore ternario costa 0.01 ms sul fingerprints
...
public:
	...
	void PerformLabeling() {
		...
		//PathCompression << <grid_size, block_size >> > (d_img_, d_labels_, d_global_labels);
