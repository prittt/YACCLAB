...
void YacclabTests::CheckMethodsExistence() {
    ...
    for (const auto& algo_name : mode_cfg_.ccl_existing_algorithms) {
        ...
        if (mode_cfg_.perform_blocksize || (mode_cfg_.perform_correctness && mode_cfg_.perform_check_8connectivity_bs)) {
            try {
                algorithm->PerformLabelingBlocksize(16, 16, 1);
                if (KernelMapSingleton::Exists(algo_name))
                {
                    mode_cfg_.ccl_blocksize_algorithms.push_back(algo_name);
                }
                else {
                    ob_.Cwarning("Kernels of " + algo_name + " were not registered for Blocksize test, skipped");
                }
            }
            catch (const std::runtime_error& e) {
                ob_.Cwarning(algo_name + ": " + e.what());
    ...
    if ((mode_cfg_.perform_blocksize|| (mode_cfg_.perform_correctness && mode_cfg_.perform_check_8connectivity_bs)) && mode_cfg_.ccl_blocksize_algorithms.size() == 0) {
        ob_.Cwarning("There are no 'algorithms' with valid 'PerformLabelingBlocksize()' method, related tests will be skipped");
        mode_cfg_.perform_blocksize = false;
        mode_cfg_.perform_check_8connectivity_bs = false;
    }
    ...
    if ((mode_cfg_.perform_blocksize) && mode_cfg_.blocksize_datasets.size() == 0) {
        ob_.Cwarning("There are no datasets specified for 'blocksize test', skipped");
        mode_cfg_.perform_blocksize = false;
    }
...
#if defined YACCLAB_WITH_CUDA
void YacclabTests::CheckBlocksizeParameters() {

    if (mode_cfg_.perform_blocksize) {

        if (mode_cfg_.user_blocksize_x.size() != 3) {
            mode_cfg_.perform_blocksize = false;
            ob_.Cwarning("Parameter 'blocksize.x' is not in the form [first, last, step], blocksize test skipped.");
            return;
        }
        if (mode_cfg_.user_blocksize_y.size() != 3) {
            mode_cfg_.perform_blocksize = false;
            ob_.Cwarning("Parameter 'blocksize.y' is not in the form [first, last, step], blocksize test skipped.");
            return;
        }
        if (mode_cfg_.user_blocksize_z.size() != 3) {
            mode_cfg_.perform_blocksize = false;
            ob_.Cwarning("Parameter 'blocksize.z' is not in the form [first, last, step], blocksize test skipped.");
            return;
        }

        std::copy(begin(mode_cfg_.user_blocksize_x), end(mode_cfg_.user_blocksize_x), begin(mode_cfg_.blocksize_x));
        std::copy(begin(mode_cfg_.user_blocksize_y), end(mode_cfg_.user_blocksize_y), begin(mode_cfg_.blocksize_y));
        std::copy(begin(mode_cfg_.user_blocksize_z), end(mode_cfg_.user_blocksize_z), begin(mode_cfg_.blocksize_z));
        mode_cfg_.user_blocksize_x.clear();
        mode_cfg_.user_blocksize_y.clear();
        mode_cfg_.user_blocksize_z.clear();

        for (int i = 0; i < 3; ++i) {
            if (mode_cfg_.blocksize_x[i] <= 0 || mode_cfg_.blocksize_y[i] <= 0 || mode_cfg_.blocksize_z[i] <= 0) {
                mode_cfg_.perform_blocksize = false;
                ob_.Cwarning("Blocksize parameters must be positive, blocksize test skipped.");
                return;
            }
        }

        // TODO: what if there are multiple devices?
        cudaDeviceProp prop;
        cudaGetDeviceProperties(&prop, 0);
        int max_x = prop.maxThreadsDim[0];
        int max_y = prop.maxThreadsDim[1];
        int max_z = prop.maxThreadsDim[2];
            
        for (int i = 0; i < 2; ++i) {
            if (mode_cfg_.blocksize_x[i] > max_x)
                mode_cfg_.blocksize_x[i] = max_x;
            if (mode_cfg_.blocksize_y[i] > max_y)
                mode_cfg_.blocksize_y[i] = max_y;
            if (mode_cfg_.blocksize_z[i] > max_z)
                mode_cfg_.blocksize_z[i] = max_z;
        }
