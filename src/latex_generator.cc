...
void GenerateLatexCharts(const filesystem::path& output_path, const string& latex_charts, const string& latex_folder, const vector<String>& datasets_name)
{
	...
	for (unsigned i = 0; i < datasets_name.size(); ++i) {
		os << "\t\\begin{subfigure}[b]{" + chartSize + "\\textwidth}" << endl;
		os << "\t\t\\caption{" << datasets_name[i] + "}" << endl;
		os << "\t\t\\centering" << endl;
		os << "\t\t\\includegraphics[width=" + chartWidth + "\\textwidth]{\\compilerName " + datasets_name[i] + ".pdf}" << endl;
...
void LatexGenerator(const path& output_path, const string& latex_file, const Mat1d& all_res, const vector<String>& datasets_name, const vector<String>& ccl_algorithms, const vector<String>& ccl_mem_algorithms, const map<string, Mat1d>& accesses)
{
	//string latex_path = output_path + kPathSeparator + latex_file;
	path latex_path = output_path / path(latex_file);
	ofstream os(latex_path.string());
	...
	os << "\\section{Average Results}" << endl << endl;
	...
	if (ccl_mem_algorithms.size() > 0) {
		os << "\\section{Memory tests}" << endl << endl;
