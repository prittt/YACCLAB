	...
	for (RowColType r = filaInicial; r < filaFinal; r++) {
		...
		for (RowColType c = 1; c < N_COLS; c++) {
			if ((img_row[c] == BG)) {
				...
				JumpType jump_rc = BG_lookup_table_2neighb(Le, U); //  priority: : +1 for FG;  -1 for BG  // 1� load  
				if (jump_rc != 0) {
					JumpType other_jump = *(pjump_rc + jump_rc); // 2� load  
					...
					*pjump_rc = total_jump;  // 1� store with if  /*	instead of if() version  ... see below 
				}
				else {
					*pjump_rc = 0;
				}
	...
	for (RowColType r = filaFinal - 1; r >= filaInicial; r--) {
		...
		for (int c = N_COLS - 2; c >= 0; c--) {
			if ((img_row[c] == FG)) {
				...
				JumpType *pjump_rc = &(imgJump_row[c]); //@ it is faster than pjump_rc-- ?
				JumpType jump_rc = FG_lookup_table_3neighb(R, RD, D); //  priority: : +1 for FG;  -1 for BG ;  // 1� load  

				if (jump_rc != 0) {
					JumpType other_jump = *(pjump_rc + jump_rc); // 2� load  
					JumpType total_jump = jump_rc + (other_jump);  // NO LOAD   
					*pjump_rc = total_jump;  // 1� store with if  /*	instead of if() version 
				}
				else {
					*pjump_rc = 0;
...
// this is a more guided version to help the compiler to avoid load/stores. Only 2 loads and one store for each change 
   ...
   // forward travelling  or run promotes BG pixel jumps in a sequential fashion !
		for (int c = 0; c < N_COLS; c++) {
			JumpType *pjump_rc = &(imgJump_row[c]); //@ it is faster than pjump_rc++ ?
			JumpType jump_rc = *pjump_rc;  // 1� load  
			JumpType other_jump = *(pjump_rc + jump_rc); // 2� load  
			  // a little faster using conditional code here
			int cond1 = (other_jump != 0);
			...
			JumpType total_jump = jump_rc + (other_jump);  // NO LOAD   
			*pjump_rc = total_jump;  // 1� store with if 
		}
...
unsigned Labelling_only(const cv::Mat1b* I, cv::Mat1i* J, cv::Mat1i* L, int id, int numHilos) {
	...
	for (int r = filaInicial; r < filaFinal; r++) {    // FG critical cells at the SOUTH (BOTTOM) border are considered as real critical cells
		...
		for (int c = 0; c < N_COLS; c++) {  // FG critical cells at the East (RIGHT) border are considered as real critical cells
			...
			{  // 2� load  // Labels L for possible critical cells were written at the init()
#ifdef DEBUG_INNER_STATISTICS
				...
				JumpType new_jump = *(pnew_jump); //  3� load  //&
				...
				*pLabels_rc = label_rc_00 + sum_jumps_rc; //& //1� store @ this must be recomputed according to the nof threads 
														  // also sth like: L[r][c] = (pjump_rc - &J[0][0] ) + jump_rc; //1� store @ this must be recomputed according to the nof threads 
...
#pragma omp parallel reduction (+:nof_FG_transports)reduction (+:nof_BG_transports) shared ( I)
	{
		...
		// Get rows pointers
			...
			if (jump_rc == 0) {  // 2� load  if it were a FG critical cell

				if (((c + C_SHIFT_SEARCH) < N_COLS) && ((r + R_SHIFT_SEARCH) < N_ROWS)) {   //(( @ NO BORDERS //%CHECK 1
					JumpType other_color_jump = *(pjump_rc + C_SHIFT_SEARCH + N_COLS * R_SHIFT_SEARCH); // 3� load  
																					  ...
																					  //@ it could be not necessary (because all the checking is done 3 lines below ) 
					{
						...
						{ //(((@ NO_BORDERS_ADDITIONAL 
						  ...
						  // normal case: not in   borders:
							other_color_jump_backwards = *(pnew_other_color_crit_cell - C_SHIFT_SEARCH - N_COLS * R_SHIFT_SEARCH); // 4� load  
							...
							if (pnew_other_color_jump_backwards == pjump_rc) {  // do the transport:
								...
								if (((c - C_SHIFT_NEW_ROOT) < N_COLS) && ((r - R_SHIFT_NEW_ROOT) < N_ROWS)) { //(((@ NO BORDERS  //%CHECK 4a
									...
									JumpType * pother_color_new_root_origin = (pjump_rc - C_SHIFT_NEW_ROOT - N_COLS * R_SHIFT_NEW_ROOT); // 5� load  
									...
									JumpType new_value = static_cast<JumpType>(sum_jump_other_color_new_root + (pother_color_new_root_origin - pnew_other_color_crit_cell));	// 2� store with if 

									*pnew_other_color_crit_cell = new_value; // 2� store with if 
								...
								{ //(((@ NO BORDERS //%CHECK 4b
								  //the deleted FG crit. cell is written with
									JumpType * pnew_root_origin = (pnew_other_color_crit_cell + C_SHIFT_NEW_ROOT + N_COLS * R_SHIFT_NEW_ROOT); // 5� load  
									...
									*pjump_rc = new_value; //// 1� store with if �
				...
				} // endof if (jump_rc == 0 && I[r][c] == FG) {  // 2� load  if it were a FG critical cell
			}
		} // endof   for (int r = 0; r < N_ROWS; r++)  
	}
